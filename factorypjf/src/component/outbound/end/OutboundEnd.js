  ...
  const grid01_headers = [
      ...
      { text: "입고일", value: "itemName", width: "9%", helper: true },
  ];
  ...
  const handleMasterSearch = async () => {
    try {
      const response = await api.post('/outbound/searchMaster', searchSectionData.master);
  ...
  const handleDetailSearch = async () => {
    try {
      const response = await api.post('/inbound/searchDetail', searchSectionData.detail);
      setSearchData(response.data.data);
      setSelectedBoundId(null); // 추가
    } catch (error) {
      console.error('Error sending master data:', error);
  ...
  useEffect(()=>{
      const filteredData = outboundAll?.data?.filter(data => data.bound_state === 'end');
    ...
    return (
      <div className={inboundClasses.wrap}>
          ...
          style={{display:'flex'}}>
            <div>문서번호</div>
            <div><input type="text" value={searchSectionData.master.bound_no} onChange={(e) => handleInputChange('master', 'bound_no', e.target.value)} /></div>
            <div>유형</div>
            <div>
            ...
            <div>거래처명</div>
            <div><input type="text" value={searchSectionData.master.partner_code} /></div>
            <button onClick={(e) => {
                  setSelectedHelperItem(item); 
                  sedivelperScreenState(!HelperScreenState);
            }}>?</button>
            <div>입고일</div>
            <div><input type="date" value={searchSectionData.master.bound_start_date} onChange={(e) => handleInputChange('master', 'bound_start_date', e.target.value)} /></div>
            <div>~</div>
            <div><input type="date" value={searchSectionData.master.bound_end_date} onChange={(e) => handleInputChange('master', 'bound_end_date', e.target.value)} /></div>
            <div><button onClick={handleMasterSearch}>Master 검색</button></div>
            <h3>/</h3>
            <div>품목명</div>
            <div><input type="text" value={searchSectionData.detail.item_name}/></div>
            <button onClick={(e) => {
                  setSelectedHelperItem(item2); 
                  sedivelperScreenState(!HelperScreenState);
            }}>?</button>
            <div>품목입고일</div>
            <div><input type="date" value={searchSectionData.detail.detail_start_date} onChange={(e) => handleInputChange('detail', 'detail_start_date', e.target.value)} /></div>
            <div>~</div>
            <div><input type="date" value={searchSectionData.detail.detail_end_date} onChange={(e) => handleInputChange('detail', 'detail_end_date', e.target.value)} /></div>
            <div><button onClick={handleDetailSearch}>Detail 검색</button></div>
          </div>
