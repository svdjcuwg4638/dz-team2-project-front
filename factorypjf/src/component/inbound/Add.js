...
import InboundAddTable from './table/InboundAddTable';

function Add() {

  const [gridData,setgridData] = useState([]);

  const updateGridData = (data, rowIndex) => {
      ...
      <div className={inboundClasses.wrap}>
        ...
        <div className={inboundClasses.grid01}>
          <Table headers={grid01_headers}>
            <InboundAddTable headers={grid01_headers}/>
          </Table>
        </div>
        <div className={inboundClasses.grid02}>
          <Table headers={grid02_headers}>
            <InboundAddTable headers={grid02_headers}/>
          </Table>
        </div>
        <button>삭제</button>
        <button>저장</button>
